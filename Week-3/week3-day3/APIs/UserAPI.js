...
// if password matches
  ...
  const signedToken=sign({email:user.email},process.env.SECRET_KEY,{expiresIn:"5h"})  // sign({email:user.email}, Payload for every diff user
...
userApp.get("/user",verifyToken, async (req, res) => {
    ...
    // add product to cart
    // before adding first it should check that product is already in the cart
    // if product is there increment by one
    // otherwise add that product to cart
   


    // find user by id  populate it will do all the propertis of a product in  a get user

    const userObj = await UserModel.findOne({email:emailOfUser}).populate("cart.product") //product is ref
    // if user not found 
    if(!userObj){
